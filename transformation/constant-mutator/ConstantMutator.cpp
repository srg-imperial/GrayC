...
bool ConstantMutatorVisitor::VisitVarDecl(VarDecl *D) {
  if (auto t = dyn_cast_or_null<ConstantArrayType>(D->getType().getTypePtr())) {
    latest_array_line_no = ((m_astContext->getSourceManager())
...
bool ConstantMutatorVisitor::VisitFloatingLiteral(FloatingLiteral *flit) {
  ...
  string const_str =
      string(Lexer::getSourceText(const_range, m_astContext->getSourceManager(),
  ...
  if (point_loc != std::string::npos) {
    ...
    string mutated_float = to_string(mutate_constant_float()) + "." +
...
bool ConstantMutatorVisitor::VisitIntegerLiteral(IntegerLiteral *ilit) {
  ...
  if (const_str != "" && is_number(const_str)) {
    m_rewriter->ReplaceText(const_range,
                            "(" + mutate_constant_integers(const_str) + ")");
  }
  return true;
}
...
cType ConstantMutatorVisitor::guessType(std::string const_str,
  ...
  if (is_zero) {
    return ((cType)(ClangFuzzerCustomRandom::GetInstance()->rnd_dice() %
                    MAX_CONST_TYPE));
  }
...
char ConstantMutatorVisitor::bit_flip_char(char constant) {
  ...
  unsigned bit_flip_location =
      ClangFuzzerCustomRandom::GetInstance()->rnd_dice(bits.size());
...
std::string ConstantMutatorVisitor::replace1char(std::string const_str) {
  ...
  for (unsigned i = 0; i < len; i++) {
    if (std::isdigit(const_str[i]) &&
        ClangFuzzerCustomRandom::GetInstance()->rnd_yes_no(0.1)) {
      const_str[i] =
          dec_characters[ClangFuzzerCustomRandom::GetInstance()->rnd_dice(10)];
      return const_str;
    }
  }
  return const_str;
...
void ConstantMutatorFrontendAction::EndSourceFileAction() {
  ...
  std::string base_filename =
      string(src_mngr.getFileEntryForID(src_mngr.getMainFileID())->getName());
...
{
  static llvm::cl::OptionCategory ConstantMutatorCategory("Transform tool option");
  static llvm::cl::opt<std::string> OutputFile("output",
                                              llvm::cl::desc("Name of the output file"),
                                              llvm::cl::cat(ConstantMutatorCategory), cl::Required);
  static llvm::cl::opt<unsigned long> Seed("seed",
                                              llvm::cl::desc("String seed to feed into the mutator"),
                                              llvm::cl::cat(ConstantMutatorCategory), cl::Required);

  llvm::cl::HideUnrelatedOptions(ConstantMutatorCategory);
  CommonOptionsParser op(argc, argv, ConstantMutatorCategory);
