...
class ConstantMutatorVisitor : public RecursiveASTVisitor<ConstantMutatorVisitor> {
private:
    ...
    static long long sign_flip_long_long(long long constant) { return (~constant); }
...
public:
    ...
    explicit ConstantMutatorASTConsumer(CompilerInstance* CI, Rewriter* rewriter)
        : m_visitor(new ConstantMutatorVisitor(CI, rewriter)) // initialize the visitor
    { }
...
public:
    virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(CompilerInstance& CI, StringRef file) final {
        //return std::unique_ptr<clang::ASTConsumer>(new ConstantMutatorASTConsumer(&CI, m_rewriter)); // pass CI pointer to ASTConsumer
