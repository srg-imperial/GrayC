...
public:
    ...
    explicit JumpMutatorASTConsumer(CompilerInstance* CI, Rewriter* rewriter)
        : m_visitor(new JumpMutatorVisitor(CI, rewriter)) // initialize the visitor
    { }
...
public:
    virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(CompilerInstance& CI, StringRef file) final {
        //return std::unique_ptr<clang::ASTConsumer>(new JumpMutatorASTConsumer(&CI, m_rewriter)); // pass CI pointer to ASTConsumer
