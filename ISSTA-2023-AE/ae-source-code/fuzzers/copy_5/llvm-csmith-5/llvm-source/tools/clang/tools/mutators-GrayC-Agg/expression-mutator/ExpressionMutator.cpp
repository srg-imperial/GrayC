...
/// 
/// Classes: ExpressionMutatorVisitor, ExpressionMutatorASTConsumer, ExpressionMutatorFrontendAction
/// </summary>
// Write up in a mail
...
void ExpressionMutatorVisitor::collect_sub_expressions(BinaryOperator *stmt) {
    if (!stmt->isCommaOp() && !stmt->isPtrMemOp()) {
        ...
        //bool is_rhs_pointer = sub_expr_r->getType()->isAnyPointerType() || sub_expr_r->getType()->isStructureType() || sub_expr_r->getType()->isRecordType() || sub_expr_r->getType()->isVectorType();
        bool is_rhs_arithmetic = sub_expr_r->getType()->isArithmeticType();
        bool is_lhs_arithmetic = sub_expr_l->getType()->isArithmeticType();
        //bool is_lhs_pointer = sub_expr_l->getType()->isAnyPointerType() || sub_expr_l->getType()->isStructureType() || sub_expr_l->getType()->isRecordType() || sub_expr_l->getType()->isVectorType();
        ...
        std::string sub_decl_str_r = std::string(Lexer::getSourceText(sub_declRange_r, m_astContext->getSourceManager(), m_astContext->getLangOpts()));
        CharSourceRange sub_declRange_l = CharSourceRange::getTokenRange(sub_expr_l->getBeginLoc(), sub_expr_l->getEndLoc());
        std::string sub_decl_str_l = std::string(Lexer::getSourceText(sub_declRange_l, m_astContext->getSourceManager(), m_astContext->getLangOpts()));
...
bool ExpressionMutatorVisitor::VisitBinaryOperator(BinaryOperator *stmt) {
    ...
    bool isVisited = (std::find(visited_node_IDs.begin(), visited_node_IDs.end(), binary_op_node_id) != visited_node_IDs.end());
    CharSourceRange binop_range = CharSourceRange::getTokenRange(stmt->getBeginLoc(), stmt->getEndLoc());
    std::string bstmt_str = std::string(Lexer::getSourceText(binop_range, m_astContext->getSourceManager(), m_astContext->getLangOpts()));
    if (stmt->isAssignmentOp() && !isVisited) {
        ...
        int expr_length = (ClangFuzzerCustomRandom::GetInstance()->rnd_dice() % _MUTATOR_EXPR_SIZE) + 1;
...
bool ExpressionMutatorVisitor::VisitIfStmt(IfStmt *istmt){
    ...
    std::string cond_str = std::string(Lexer::getSourceText(cond_range, m_astContext->getSourceManager(), m_astContext->getLangOpts()));
    ...
    if (expr_to_add.size()) {
        ...
        int expr_length = (ClangFuzzerCustomRandom::GetInstance()->rnd_dice() % _MUTATOR_SUB_EXPR_SIZE) + 1;
        for (int i=0;i<expr_length;i++)
            expr_append += get_float_random_op() + "((" + get_random_dtype() + ")(" + generate_random_expr() + "))";
...
std::string ExpressionMutatorVisitor::generate_random_expr(){
    std::string expr_lhs = expr_to_add[ClangFuzzerCustomRandom::GetInstance()->rnd_dice() % expr_to_add.size()];
    std::string expr_rhs = expr_to_add[ClangFuzzerCustomRandom::GetInstance()->rnd_dice() % expr_to_add.size()];

    bool lhs_pointer = (expr_lhs.find("*")!=std::string::npos) || (expr_lhs.find("[")!=std::string::npos);
    bool rhs_pointer = (expr_rhs.find("*")!=std::string::npos) || (expr_rhs.find("[")!=std::string::npos);
	std::string random_expr = (ClangFuzzerCustomRandom::GetInstance()->rnd_yes_no(0.5) && !lhs_pointer && !rhs_pointer)
...
std::string ExpressionMutatorVisitor::get_float_random_op(){
    return (float_operator_bank[ClangFuzzerCustomRandom::GetInstance()->rnd_dice() % _MUTATOR_FLOAT_OP_SIZE]);
}

std::string ExpressionMutatorVisitor::get_int_random_op(){
    return (int_operator_bank[ClangFuzzerCustomRandom::GetInstance()->rnd_dice() % _MUTATOR_INT_OP_SIZE]);
}

std::string ExpressionMutatorVisitor::get_random_dtype(){
    return (data_types[ClangFuzzerCustomRandom::GetInstance()->rnd_dice() % _MUTATOR_DATA_TYPE_SIZE]);
}
...
int main(int argc, const char **argv) {
    ...
    int result = Tool.run(&(*newFrontendActionFactory<ExpressionMutatorFrontendAction>()));
