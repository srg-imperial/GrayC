...
std::vector<std::string> FuzzerArgument::_mutator_options = {
    "bin/append-expression",  "bin/jump-mutator",      "bin/function-merger",
...
std::string FuzzerArgument::_fuzzer_remove_parens =
    "(ulimit -Sf 2000 ; ulimit -St 30 ; /home/user42/fuzzers/remove-parens/build/remove-parens";
  ...
  if (is_mutation_multi_file && sanity_checks) {
    ...
    if (Size) {
      result_libtooling = std::system(
          std::string("python /home/user42/fuzzers/copy_5/run-um.py " + temp_file_name).c_str());
    } else {
      return Size;
    }
...
string rawname = temp_file_name.substr(0, lastindex);
      rawname =
          rawname.substr(rawname.find_last_of("//")+1, rawname.length());

      // Load mutated program from file
      //std::cerr<<"ar89\n";
      std::cerr<<temp_file_name<<"\n";
      std::cerr<<rawname+".mutated.c"<<"\n";
      //std::ifstream in(rawname+".mutated.c");
      rawname = "/home/user42/fuzzers/copy_5/llvm-csmith-5/llvm-fuzzer-build/fd/" + rawname;
      std::ifstream in(rawname+".mutated.c");
      std::cerr<<rawname<<"\n";
      ...
      in.close();
      std::string rmd = "rm -rf /home/user42/fuzzers/copy_5/llvm-csmith-5/llvm-fuzzer-build/fd/";
      system(rmd.c_str());
    }
