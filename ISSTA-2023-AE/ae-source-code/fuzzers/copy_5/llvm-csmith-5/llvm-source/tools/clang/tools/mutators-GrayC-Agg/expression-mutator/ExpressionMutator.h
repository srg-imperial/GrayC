...
#define _MUTATOR_MAX_EXPR_SIZE 200
...
class ExpressionMutatorVisitor : public RecursiveASTVisitor<ExpressionMutatorVisitor> {
private:
    ...
    std::string data_types[_MUTATOR_DATA_TYPE_SIZE] = { "int","long","short" }; // add short long and unsigned
...
public:
    ...
    explicit ExpressionMutatorASTConsumer(CompilerInstance* CI, Rewriter* rewriter)
        : m_visitor(new ExpressionMutatorVisitor(CI, rewriter)) // initialize the visitor
    { }

    // override this to call our ExpressionMutatorVisitor on the entire source file
...
public:
    virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(CompilerInstance& CI, StringRef file) final {
        //return std::unique_ptr<clang::ASTConsumer>(new ExpressionMutatorASTConsumer(&CI, m_rewriter)); // pass CI pointer to ASTConsumer
