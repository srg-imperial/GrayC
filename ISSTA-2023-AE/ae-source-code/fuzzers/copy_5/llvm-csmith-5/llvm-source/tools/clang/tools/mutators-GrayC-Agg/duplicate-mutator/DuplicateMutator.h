...
class DuplicateMutatorVisitor : public RecursiveASTVisitor<DuplicateMutatorVisitor> {
private:
...
public:
    ...
    bool VisitWhileStmt(WhileStmt* stmt) {
        latest_loop_line = (m_astContext->getSourceManager()).getSpellingLineNumber(stmt->getBeginLoc());
        return true;
    }
...
public:
    ...
    explicit DuplicateMutatorASTConsumer(CompilerInstance* CI, Rewriter* rewriter)
        : m_visitor(new DuplicateMutatorVisitor(CI, rewriter)) // initialize the visitor
    { }
...
public:
    virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(CompilerInstance& CI, StringRef file) final {
        //return std::unique_ptr<clang::ASTConsumer>(new DuplicateMutatorASTConsumer(&CI, m_rewriter)); // pass CI pointer to ASTConsumer
        return std::make_unique<DuplicateMutatorASTConsumer>(&CI, m_rewriter); // KEM: you should not use new in this method at all. 
    }
