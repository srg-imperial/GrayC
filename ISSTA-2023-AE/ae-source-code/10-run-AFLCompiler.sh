# Run No Fuzz Fuzzer with their artifact mutator
#!/bin/bash
...
seeds=/home/user42/seeds/seeds_out10.txt
...
end_ts=`date -d '+1 day +0 hour +0 minutes' '+%F %T'`
echo "Starts No Fuzz Fuzzer $curr_ts and will end at $end_ts"


# Use this script to run No Fuzz Fuzzer for 24 Hours, or any other fuzzer that is LibFuzzer based
cd $build
...
while IFS= read -r seed ; do
        ...
        echo "Running No Fuzz Fuzzer with seed $seed $timestamp"
        (bin/nofuss $setA -detect_leaks=0 -max_len=1000000 -rss_limit_mb=4096 -create_missing_dirs=1 -ignore_crashes=1 -ignore_timeouts=1 -ignore_ooms=1 -seed=$seed -timeout=50 -max_total_time=86400)
...
echo "DONE: bin/nofuss /home/user42/fuzzers/copy_5/llvm-csmith-5/../setA/ ... "
