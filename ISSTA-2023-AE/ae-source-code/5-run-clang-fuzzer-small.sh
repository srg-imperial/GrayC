...
echo "Starts Clang-Fuzzer $curr_ts and will end at $end_ts"
end_total=`date -d "$end_ts" +%s`

# Use this script to run Clang-Fuzzer for 24 Hours, or any other fuzzer that is LibFuzzer based
cd $build
...
while IFS= read -r seed ; do
  ...
  echo "Running Clang-Fuzzer with seed $seed $timestamp"
  (bin/clang-fuzzer $setA -detect_leaks=0 -max_len=1000000 -rss_limit_mb=4096 -create_missing_dirs=1 -ignore_crashes=1 -ignore_timeouts=1 -ignore_ooms=1 -seed=$seed -timeout=50 -max_total_time=600)
  ...
  rm $build/fuzzer-output-directory-prev/A_*
  ...
  if [ "$curr_total" -ge "$end_total" ] ; then
    echo "DONE: bin/clang-fuzzer $setA ... "
    exit
  fi
done < "$seeds"
echo "DONE: bin/clang-fuzzer $setA ... "
