...
#  ! grep 'uninitialized' outb.txt &&\
  ...
  ! $gcc -O2 -pedantic -Wall -Wextra -I/home/user42/data/csmith/runtime/ -I/home/user42/data/csmith/build/runtime/ $src -o gcc.out >outa.txt 2>&1 &&\
  cat outa.txt | grep "Please submit a full bug report" &&\
  cat outa.txt | grep "generic_simplify_BIT_NOT_EXPR" &&\
