#!/bin/bash
clang=clang-14
gcc=/home/user42/data/gcc-csmith-112/gcc-install/bin/gcc
src=fuzzer-file-265231.c
...
$clang $opt -fsanitize=undefined -g -pedantic -Wall -Wextra -I/home/user42/data/csmith/runtime/ -I/home/user42/data/csmith/build/runtime/ $src -o clang.out >outb.txt 2>&1 &&\
  ...
  cat outa.txt | grep "gcc: internal compiler error: Segmentation fault signal terminated program cc1" 
