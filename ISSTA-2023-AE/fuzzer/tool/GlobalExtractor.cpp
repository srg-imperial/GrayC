...
class GlobalExtractorVisitor
    : public RecursiveASTVisitor<GlobalExtractorVisitor> {
private:
...
public:
  ...
  bool VisitFunctionDecl(FunctionDecl *func) {
    if (func->isMain()) {
      string func_text = string(Lexer::getSourceText(
  ...
  bool VisitVarDecl(clang::VarDecl *NamedDecl) {
    if (NamedDecl->hasGlobalStorage() && !(NamedDecl->isStaticLocal())) {
      string decl_str = NamedDecl->getNameAsString() + "\t" +
                        NamedDecl->getType().getAsString();
      llvm::outs() << decl_str << "\n";
  ...
  bool VisitRecordDecl(clang::RecordDecl *rd) {
    ...
    for (RecordDecl::field_iterator j_se = rd->field_begin(),
      ...
      string decl_str =
          decl->getNameAsString() + "\t" + decl->getType().getAsString();
...
public:
  ...
  explicit GlobalExtractorASTConsumer(CompilerInstance *CI, Rewriter *rewriter)
      : m_visitor(
            new GlobalExtractorVisitor(CI, rewriter)) // initialize the visitor
  {}
...
public:
  ...
  CreateASTConsumer(CompilerInstance &CI, StringRef file) final {
    return std::unique_ptr<clang::ASTConsumer>(new GlobalExtractorASTConsumer(
        &CI, m_rewriter)); // pass CI pointer to ASTConsumer
  }
...
int main(int argc, const char **argv) {
  ...
  int result =
      Tool.run(&(*newFrontendActionFactory<GlobalExtractorFrontendAction>()));
