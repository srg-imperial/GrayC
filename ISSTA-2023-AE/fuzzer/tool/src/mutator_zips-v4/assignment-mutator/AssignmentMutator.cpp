...
/// 
/// Classes: AssignmentMutatorVisitor, AssignmentMutatorASTConsumer, AssignmentMutatorFrontendAction
/// </summary>
...
bool AssignmentMutatorVisitor::VisitUnaryOperator(UnaryOperator* stmt) {
    ...
    if (ClangFuzzerCustomRandom::GetInstance()->rnd_yes_no(0.1)) {
        ...
        std::string decl_str = string(Lexer::getSourceText(decl_range, m_astContext->getSourceManager(), m_astContext->getLangOpts()));
...
void AssignmentMutatorFrontendAction::EndSourceFileAction() {
    ...
    std::string base_filename = string(SM.getFileEntryForID(SM.getMainFileID())->getName());
...
int main(int argc, const char **argv) {
	...
	int result = Tool.run(&(*newFrontendActionFactory<AssignmentMutatorFrontendAction>()));
