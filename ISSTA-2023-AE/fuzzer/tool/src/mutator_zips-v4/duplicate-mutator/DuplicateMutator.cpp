...
/// 
/// Classes: DuplicateMutatorVisitor, DuplicateMutatorASTConsumer, DuplicateMutatorFrontendAction
/// </summary>
// Write up in a mail
...
bool DuplicateMutatorVisitor::VisitBinaryOperator(BinaryOperator* stmt) {
    int line_no = (m_astContext->getSourceManager()).getSpellingLineNumber(stmt->getBeginLoc());
    if (stmt->isAssignmentOp() && latest_loop_line != line_no) {
        ...
        std::string bstmt_str = string(Lexer::getSourceText(binop_range, m_astContext->getSourceManager(), m_astContext->getLangOpts()));
...
std::string DuplicateMutatorVisitor::pick_delete_or_duplicate() { 
    return (ClangFuzzerCustomRandom::GetInstance()->rnd_yes_no(0.5) ? "duplicate" : "delete");
}
...
int main(int argc, const char **argv) {	     
    ...
    int result = Tool.run(&(*newFrontendActionFactory<DuplicateMutatorFrontendAction>()));
