...
public:
    ...
    explicit DeleteMutatorASTConsumer(ASTContext* astContext, Rewriter* rewriter)
        : m_visitor(new DeleteMutatorVisitor(astContext, rewriter)) // initialize the visitor
    { }
...
public:
    virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(CompilerInstance& CI, StringRef file) final {
        ...
        //return std::unique_ptr<clang::ASTConsumer>(new DeleteMutatorASTConsumer(m_astContext, m_rewriter));
        return std::make_unique<DeleteMutatorASTConsumer>(m_astContext, m_rewriter);
    }

    virtual ~DeleteMutatorFrontendAction() { delete m_rewriter; m_astContext = 0; }
