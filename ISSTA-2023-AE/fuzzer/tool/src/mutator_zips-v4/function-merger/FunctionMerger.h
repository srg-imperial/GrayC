...
class FunctionCombinerVisitor : public RecursiveASTVisitor<FunctionCombinerVisitor> {
private:
...
public:
    ...
    explicit FunctionCombinerASTConsumer(CompilerInstance* CI, Rewriter* rewriter)
        : m_visitor(new FunctionCombinerVisitor(CI, rewriter)) // initialize the visitor
    { }
...
public:
    virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(CompilerInstance& CI, StringRef file) final {
        //return std::unique_ptr<clang::ASTConsumer>(new FunctionCombinerASTConsumer(&CI, m_rewriter)); // pass CI pointer to ASTConsumer
