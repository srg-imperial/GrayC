...
/// 
/// Classes: FunctionCombinerVisitor, FunctionCombinerASTConsumer, FunctionCombinerFrontendAction
/// </summary>
...
bool FunctionCombinerVisitor::VisitFunctionDecl(clang::FunctionDecl *func) {
    ...
    if ((func->isMain())) {
        ...
        if (!replace_func_text.empty()) 
            m_rewriter->InsertTextBefore(func->getBeginLoc(), replace_func_text + "\n");
    }
    return true;
}
...
std::string FunctionCombinerVisitor::getCandidateMutatingFunctionMap() {
    ...
    if (file.is_open()) {
        ...
        while (true) {
            ...
            if (pos != std::string::npos) // Can find funcName in funcBody (first position)
...
int main(int argc, const char **argv) {
    ...
    int result = Tool.run(&(*newFrontendActionFactory<FunctionCombinerFrontendAction>()));
