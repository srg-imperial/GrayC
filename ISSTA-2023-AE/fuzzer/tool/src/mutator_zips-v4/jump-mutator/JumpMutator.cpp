...
/// 
/// Classes: JumpMutatorVisitor, JumpMutatorASTConsumer, JumpMutatorFrontendAction
/// </summary>
// Write up in a mail
...
bool JumpMutatorVisitor::VisitWhileStmt(WhileStmt *stmt) {
    if (m_astContext->getSourceManager().isInExternCSystemHeader(stmt->getBeginLoc()))
        return true;
...
string JumpMutatorVisitor::pick_break_or_continue() {
    return (ClangFuzzerCustomRandom::GetInstance()->rnd_yes_no(0.5) ? "break" : "continue");
}

void JumpMutatorFrontendAction::EndSourceFileAction() {
    SourceManager &SM = m_rewriter->getSourceMgr();
    std::error_code error_code;
    string base_filename = string(SM.getFileEntryForID(SM.getMainFileID())->getName());
...
int main(int argc, const char **argv) {
    ...
    ClangFuzzerCustomRandom::CreateInstance(Seed, 50); // Dice of 50
    ...
    int result = Tool.run(&(*newFrontendActionFactory<JumpMutatorFrontendAction>()));
