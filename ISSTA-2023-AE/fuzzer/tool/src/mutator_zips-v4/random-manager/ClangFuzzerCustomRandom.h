...
public:
  static ClangFuzzerCustomRandom *GetInstance(void) {
    assert(m_instance);
    return m_instance;
  }
  static void CreateInstance(const unsigned long _seed,
    ...
    assert(!m_instance);
    m_instance = new ClangFuzzerCustomRandom(_seed, _DICE_SIZE);
  }
...
private:
  ...
  explicit ClangFuzzerCustomRandom(const unsigned long _seed,
                                   const unsigned int _DICE_SIZE)
