...
bool DuplicateMutatorVisitor::VisitBinaryOperator(BinaryOperator *stmt) {
  ...
  if (!m_rewriter->isRewritable(stmt->getBeginLoc()) ||
      !m_rewriter->isRewritable(stmt->getEndLoc()))
  ...
  if (pick_delete_or_duplicate() == "duplicate") {
    ...
    if (GrayCCustomRandom::GetInstance()->rnd_yes_no(
            __DUPLICATE_MUTATOR_REMOVE_RHS_OR_LHS)) {
      Expr *rhs_expr = stmt->getRHS();
      if (!rhs_expr)
        return true;

      CharSourceRange rhs_range = CharSourceRange::getTokenRange(
          rhs_expr->getBeginLoc(), rhs_expr->getEndLoc());
      if (rhs_range.isInvalid())
        return true;

      string rhs_str = string(
          Lexer::getSourceText(rhs_range, m_astContext->getSourceManager(),
      ...
      CharSourceRange lhs_range = CharSourceRange::getTokenRange(
          lhs_expr->getBeginLoc(), lhs_expr->getEndLoc());
      if (lhs_range.isInvalid())
        return true;

      string lhs_str = string(
          Lexer::getSourceText(lhs_range, m_astContext->getSourceManager(),
                               m_astContext->getLangOpts(), 0));
...
std::string DuplicateMutatorVisitor::pick_delete_or_duplicate() {
  return (GrayCCustomRandom::GetInstance()->rnd_yes_no(
              __DUPLICATE_MUTATOR_DUP_OR_DEL)
              ? "duplicate"
              : "delete");
}
