...
class AssignmentMutatorVisitor
    : public RecursiveASTVisitor<AssignmentMutatorVisitor> {
private:
  ASTContext *m_astContext; // used for getting additional AST info
  Rewriter *m_rewriter;

public:
  explicit AssignmentMutatorVisitor(CompilerInstance *CI, Rewriter *rewriter)
...
public:
  ...
  explicit AssignmentMutatorASTConsumer(CompilerInstance *CI,
                                        Rewriter *rewriter)
      : m_visitor(new AssignmentMutatorVisitor(
...
public:
  ...
  CreateASTConsumer(CompilerInstance &CI, StringRef file) final {
    ...
    // AssignmentMutatorASTConsumer(&CI, m_rewriter)); // pass CI pointer to
    // ASTConsumer
    return std::make_unique<AssignmentMutatorASTConsumer>(&CI, m_rewriter);
  }

  virtual ~AssignmentMutatorFrontendAction() { delete m_rewriter; }
