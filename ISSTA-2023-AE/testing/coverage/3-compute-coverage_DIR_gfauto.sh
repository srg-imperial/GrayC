...
for testcaseFile in $testcaseDir/* ; do
	...
	if [[ "$testcaseFile" == *".c" ]] ; then
		echo "--> PERFORMING (*.c)  <(ulimit -St 500; ${compilerInfo} $testcaseFile $compiler_flag) > basic_output.txt 2>&1>"
		(ulimit -St 500; ${compilerInfo} ${compile_line_lib_default} $testcaseFile $compiler_flag) > "basic_output.txt" 2>&1
	else
		echo "--> PERFORMING (missing *.c) <(ulimit -St 500; ${compilerInfo} $testcaseFile $compiler_flag) > basic_output.txt 2>&1>"
		mv "$testcaseFile" "$testcaseFile".c
		(ulimit -St 500; ${compilerInfo} ${compile_line_lib_default} $testcaseFile.c $compiler_flag) > "basic_output.txt" 2>&1
	fi
done
unset GCOV_PREFIX
