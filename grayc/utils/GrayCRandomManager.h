...
class GrayCRandomManager {
public:
  static GrayCRandomManager *GetInstance(void) {
    assert(m_instance);
    return m_instance;
  }
  static void CreateInstance(const unsigned long _seed,
    ...
    assert(!m_instance);
    m_instance = new GrayCRandomManager(_seed, _DICE_SIZE);
  }

  static void DeleteInstance(const unsigned long _seed);
  virtual ~GrayCRandomManager(void) {}
...
private:
  static GrayCRandomManager *m_instance;
  ...
  explicit GrayCRandomManager(const unsigned long _seed,
                                   const unsigned int _DICE_SIZE)
      : m_seed(_seed), m_DICE_SIZE(_DICE_SIZE), m_rng(m_seed), m_dist(0.0, 1.0),
        m_dice_generator(0, _DICE_SIZE - 1) {}

  // Don't implement them
  GrayCRandomManager(GrayCRandomManager const &) = delete;
  void operator=(GrayCRandomManager const &) = delete;
};
