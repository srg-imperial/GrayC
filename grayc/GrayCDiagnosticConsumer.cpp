//===--- graycGrayCDiagnosticConsumer.cpp ----------=== //
//
...
//===----------------------------------------------------------------------===//
///
///  \file This file implements GrayCDiagnosticConsumer, GrayCContext
///  and GrayCError classes.
///
...
using namespace grayc;

namespace {
class GrayCDiagnosticRenderer : public DiagnosticRenderer {
public:
  GrayCDiagnosticRenderer(const LangOptions &LangOpts,
...
protected:
  void emitDiagnosticMessage(FullSourceLoc Loc, PresumedLoc PLoc,
    ...
    // appending the check name to the message in GrayCContext::diag and
...
GrayCError::GrayCError(StringRef CheckName,
                               GrayCError::Level DiagLevel,
                               StringRef BuildDirectory, bool IsWarningAsError)
    : tooling::Diagnostic(CheckName, DiagLevel, BuildDirectory),
      IsWarningAsError(IsWarningAsError) {}

class GrayCContext::CachedGlobList {
public:
...
GrayCContext::~GrayCContext() = default;

DiagnosticBuilder GrayCContext::diag(
...
void GrayCContext::setSourceManager(SourceManager *SourceMgr) {
  DiagEngine->setSourceManager(SourceMgr);
}

void GrayCContext::setCurrentFile(StringRef File) {
...
void GrayCContext::setASTContext(ASTContext *Context) {
  DiagEngine->SetArgToStringFn(&FormatASTNodeDiagnosticArgument, Context);
  LangOpts = Context->getLangOpts();
}

const GrayCGlobalOptions &GrayCContext::getGlobalOptions() const {
  return OptionsProvider->getGlobalOptions();
}

const GrayCOptions &GrayCContext::getOptions() const {
  return CurrentOptions;
}

GrayCOptions GrayCContext::getOptionsForFile(StringRef File) const {
  // Merge options on top of getDefaults() as a safeguard against options with
  // unset values.
  return GrayCOptions::getDefaults().mergeWith(
      OptionsProvider->getOptions(File), 0);
}

void GrayCContext::setEnableProfiling(bool P) { Profile = P; }

void GrayCContext::setProfileStoragePrefix(StringRef Prefix) {
...
bool GrayCContext::isCheckEnabled(StringRef CheckName) const {
  assert(CheckFilter != nullptr);
  return CheckFilter->contains(CheckName);
}

bool GrayCContext::treatAsError(StringRef CheckName) const {
  assert(WarningAsErrorFilter != nullptr);
  return WarningAsErrorFilter->contains(CheckName);
}

std::string GrayCContext::getCheckName(unsigned DiagnosticID) const {
...
GrayCDiagnosticConsumer::GrayCDiagnosticConsumer(
    GrayCContext &Ctx, DiagnosticsEngine *ExternalDiagEngine,
...
void GrayCDiagnosticConsumer::finalizeLastError() {
  if (!Errors.empty()) {
    GrayCError &Error = Errors.back();
    if (!Context.isCheckEnabled(Error.DiagnosticName) &&
        Error.DiagLevel != GrayCError::Error) {
...
static bool IsNOLINTFound(StringRef NolintDirectiveText, StringRef Line,
                          unsigned DiagID, const GrayCContext &Context) {
...
static OPTIONAL(StringRef) getBuffer(const SourceManager &SM, FileID File,
  ...
  // const clang::SrcMgr::SLocEntry &Entry = SM.getSLocEntry(File, &CharDataInvalid);
  // if (CharDataInvalid || !Entry.isFile())
  //   return llvm::None;
  // // const clang::SrcMgr::ContentCache Cache = Entry.getFile().getContentCache();
  // const llvm::MemoryBufferRef Buffer = SM.getBufferOrNone(File,SourceLocation());
  //     // AllowIO ? (Entry.getFile().getContentCache()).getBuffer(SM.getDiagnostics(), SM.getFileManager(),
...
void GrayCDiagnosticConsumer::HandleDiagnostic(
  ...
  if (ExternalDiagEngine) {
    ...
    // GrayCPluginAction case, forward the diagnostics to it.
    forwardDiagnostic(Info);
  } else {
    GrayCDiagnosticRenderer Converter(
...
bool GrayCDiagnosticConsumer::passesLineFilter(StringRef FileName,
...
void GrayCDiagnosticConsumer::forwardDiagnostic(const Diagnostic &Info) {
...
void GrayCDiagnosticConsumer::checkFilters(SourceLocation Location,
...
llvm::Regex *GrayCDiagnosticConsumer::getHeaderFilter() {
  if (!HeaderFilter)
    HeaderFilter =
...
void GrayCDiagnosticConsumer::removeIncompatibleErrors() {
  ...
  std::vector<
      std::pair<GrayCError *, llvm::StringMap<tooling::Replacements> *>>
      ErrorFixes;
  ...
  for (unsigned I = 0; I < ErrorFixes.size(); ++I) {
    if (!Apply[I]) {
      ...
      ErrorFixes[I].first->Notes.emplace_back(
          "this transformation will not be applied because it overlaps with another transformation");
    }
  }
}

namespace {
struct LessGrayCError {
  bool operator()(const GrayCError &LHS, const GrayCError &RHS) const {
...
std::vector<GrayCError> GrayCDiagnosticConsumer::take() {
  finalizeLastError();

  llvm::sort(Errors, LessGrayCError());
  Errors.erase(std::unique(Errors.begin(), Errors.end(), EqualGrayCError()),
               Errors.end());
...
namespace {
struct LessGrayCErrorWithoutDiagnosticName {
  bool operator()(const GrayCError *LHS, const GrayCError *RHS) const {
    const tooling::DiagnosticMessage &M1 = LHS->Message;
    const tooling::DiagnosticMessage &M2 = RHS->Message;
...
void GrayCDiagnosticConsumer::removeDuplicatedDiagnosticsOfAliasCheckers() {
  using UniqueErrorSet =
      std::set<GrayCError *, LessGrayCErrorWithoutDiagnosticName>;
  ...
  while (IT != Errors.end()) {
    ...
    if (Inserted.second) {
      ...
      GrayCError &ExistingError = **Inserted.first;
